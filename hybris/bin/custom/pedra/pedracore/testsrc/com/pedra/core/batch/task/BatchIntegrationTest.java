 ...
 *  
 */
package com.pedra.core.batch.task;
...
public class BatchIntegrationTest extends ServicelayerTest
{
	...
	/*
	 * @Test public void testVariant() throws Exception { processFile(productId, "base_product-", new ProductContent() {
	 * 
	 * @Override public void writeContent(final PrintWriter writer) throws IOException {
	 * writer.print("PedraSizeVariantProductModel"); super.writeContent(writer); } }); final Long variantId =
	 * Long.valueOf(Math.abs(random.nextLong())); final ProductModel product = processFile(productId, "variant-", new
	 * FileContent() {
	 * 
	 * @Override public void writeContent(final PrintWriter writer) throws IOException { writer.print(variantId);
	 * writer.print(SEPARATOR); writer.print(SEPARATOR); writer.print("black"); writer.print(SEPARATOR);
	 * writer.print("L"); } }); final PedraSizeVariantProductModel variant = (PedraSizeVariantProductModel)
	 * productService.getProductForCode(variantId .toString()); Assert.assertEquals("ApparelSizeVariantProduct",
	 * product.getVariantType().getCode()); Assert.assertEquals(product, variant.getBaseProduct());
	 * Assert.assertEquals("black", variant.getStyle(Locale.ENGLISH));
	 * Assert.assertNull(variant.getStyle(Locale.GERMAN)); Assert.assertEquals("L", variant.getSize(Locale.ENGLISH));
