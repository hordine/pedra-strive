...
public class CoreSystemSetupUnitTest
{
	...
	private void testImportSitesSync(final boolean syncAll)
	{
		...
		//electronics
		verifyProductImport(order, CoreSystemSetup.PEDRA, syncAll);

		verifyContentImport(order, CoreSystemSetup.PEDRA, syncAll);
		verifySyncContentImport(order, CoreSystemSetup.PEDRA, syncAll);

		verifySyncProductImport(order, CoreSystemSetup.PEDRA, syncAll);
		//Solr is no longer created in this way in CoreSystemSetup

		verifyAssignDependant(
				order,
				CoreSystemSetup.PEDRA + "ProductCatalog",
				new HashSet<String>(Arrays.asList(CoreSystemSetup.PEDRA + "ContentCatalog", CoreSystemSetup.PEDRA + "ContentCatalog")));
	...
	private void verifyProductImport(final InOrder order, final String catalogName, final boolean sync)
	{
		order.verify(setupImpexService).importImpexFile(
				"/pedracore/import/productCatalogs/" + catalogName + "ProductCatalog/catalog.impex", true);
	...
	private void verifySolrProductImport(final InOrder order, final String catalogName, final boolean sync) //NOPMD
	{
		if (sync)
		{
			...
			order.verify(setupImpexService).importImpexFile(
					"/pedracore/import/productCatalogs/" + catalogName + "ProductCatalog/solrtrigger.impex", true);
		}
	}
	...
	{
		order.verify(setupImpexService).importImpexFile(
				"/pedracore/import/contentCatalogs/" + catalogName + "ContentCatalog/catalog.impex", true);
		order.verify(setupImpexService).importImpexFile(
				"/pedracore/import/contentCatalogs/" + catalogName + "ContentCatalog/cms-content.impex", false);
		order.verify(setupImpexService).importImpexFile(
				"/pedracore/import/contentCatalogs/" + catalogName + "ContentCatalog/email-content.impex", false);
